...
export default function DashboardPage() {
  ...
  const [analysisPanel, setAnalysisPanel] = useState<'channels' | 'stages' | 'media'>('channels')
  ...
  const visibleDailyTotalSummary = dailyTotalSummary.filter(item => item.key <= today || item.total > 0 || item.active)
  ...
  return (
    <div className="space-y-5 p-4 md:p-6 lg:p-7">
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-3">
        ...
        <div className="card max-h-[620px] self-start overflow-auto p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-slate-800">빠른 분석</p>
              <p className="mt-1 text-[11px] text-slate-400">스크롤 없이 핵심 지표 전환</p>
            </div>
            <span className="rounded-md bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-700">총 {totalDB.toLocaleString()}건</span>
          </div>
          <div className="my-4 grid grid-cols-3 gap-1 rounded-lg bg-slate-100 p-1">
            {([
              ['channels', '유입채널'],
              ['stages', 'DB 단계'],
              ['media', 'TOP 5'],
              ...
              <button
                key={key}
                type="button"
                onClick={() => setAnalysisPanel(key)}
                className={clsx(
                  'rounded-md px-2 py-2 text-[11px] font-semibold transition-colors',
                  analysisPanel === key ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-700',
                )}
              >
                {label}
              </button>
          ...
          {analysisPanel === 'channels' && (
            <div className="space-y-3">
              {[
                ...
                return (
                  <div key={group.key} className="space-y-2 rounded-lg border border-slate-100 bg-slate-50/40 p-2">
                    <button
                      type="button"
                    ...
                    {groupOpen && group.rows.map(({ key, label, db, spend, color, details }) => (
                      <div key={key} className="space-y-1">
                        <button type="button" onClick={() => setExpandedChannel(expandedChannel === key ? null : key)} className="w-full text-left">
                          <div className="flex items-center gap-1">
                            <div className="flex-1"><ChannelBar label={label} db={db} spend={spend} maxDB={maxDB} color={color} /></div>
                        ...
                        {expandedChannel === key && details.length > 0 && (
                          <div className="mb-2 ml-5 mr-1 space-y-2 rounded-lg border border-slate-100 bg-slate-50 p-2">
                            {details.map(d => (
                              <div key={d.label} className="text-xs">
                                <div className="flex items-center justify-between"><span className="text-slate-600">{d.label}</span><span className="font-semibold text-slate-700">{d.count.toLocaleString()}건</span></div>
          ...
          {analysisPanel === 'stages' && (
            <div>
              <div className="mb-4 flex items-center justify-between text-[11px] text-slate-400"><span>선택기간 최종 상담 단계</span><span>연락처 중복 제거</span></div>
                ...
                {[
                  ...
                  return (
                    <div key={item.label}>
                      ...
                      <div className="h-1.5 rounded-full bg-slate-100"><div className={clsx('h-1.5 rounded-full', item.color)} style={{ width: `${pct}%` }} /></div>
                    </div>
                  )
                })}
              </div>
              <div className="mt-6 flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-3 py-3 text-xs">
          ...
          {analysisPanel === 'media' && (
            <div className="overflow-x-auto">
              <div className="mb-3 flex items-center justify-between text-[11px] text-slate-400"><span>DB 기여도와 매체 효율</span><span>최종 1차+2차</span></div>
              <table className="w-full text-xs">
                <thead><tr className="border-b border-slate-100 text-slate-400"><th className="pb-2.5 text-left font-medium">상세매체</th><th className="pb-2.5 text-right font-medium">광고비</th><th className="pb-2.5 text-right font-medium">DB</th><th className="pb-2.5 text-right font-medium">CPL</th></tr></thead>
                <tbody className="divide-y divide-slate-50">
                  {detailPerformance.map(row => (
                    <tr key={row.key}>
                      <td className="py-3 pr-2"><div className="font-medium text-slate-700">{row.label}</div><div className="text-[10px] text-slate-400">전환 {row.conversion}%</div></td>
                  ...
                  {!detailPerformance.length && <tr><td colSpan={4} className="py-10 text-center text-slate-400">표시할 상세매체 데이터가 없습니다.</td></tr>}
                </tbody>
              </table>
            </div>
          )}
