    ...
    reader.onload = (e) => {
      try {
        ...
        rows.forEach((row) => {
          ...
          const campaign = String(getCell(row, ['캠페인명', '캠페인', 'campaign', 'Campaign']) ?? '').trim()
          const explicitSubChannel = String(getCell(row, ['상세매체', '상세 매체', '광고상세매체', '광고 상세매체', 'detail', 'subChannel', 'sub_channel']) ?? '').trim()
          const subChannel = explicitSubChannel || inferSubChannel({ channel, source: channelRaw, campaign })
          const amountRaw = getCell(row, ['광고비', '금액', '비용', 'Cost', 'cost', 'amount', 'spend'])
          const amount = Number(String(amountRaw ?? '0').replace(/[^0-9]/g, ''))
          const memo = String(getCell(row, ['메모', '비고', 'memo', 'note']) ?? '').trim()
          const registrant = String(getCell(row, ['등록자', '작성자', '작업자', '담당자', 'registrant', 'operator', 'manager']) ?? '').trim()
          if (date && amount > 0) records.push({ date, channel, subChannel, campaign, amount, memo, registrant })
        })
        resolve({ records })
