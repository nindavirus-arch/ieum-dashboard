...
module.exports = async function salesDailyProxy(req, res) {
  ...
  if (!hasProxyAccess(req)) return sendJson(res, 403, { success: false, error: 'forbidden' })
  ...
  if (from > to) return sendJson(res, 400, { success: false, error: 'invalid range' })
  ...
  url.searchParams.set('action', 'salesDaily')
  ...
  try {
    ...
    return sendJson(res, 502, {
      success: false,
      error: 'failed to fetch sales daily report upstream',
